 ...
 * drive.
 * The PID Drive object consists of a drive object, 2 PID controllers (one for Y
 * and one for rotation) and a PID input object to provide feedback.
 */
...
public:
    ...
    {
        if (m_pidCtrlYDrive != NULL) {
            m_pidCtrlYDrive->SetTarget(distYSetPoint, m_pidInput->GetInput(m_pidCtrlYDrive));
        }
        ...
        if ((distYSetPoint == 0.0) && (angleSetPoint != 0.0)) {
        ...
        )
    {
        if (m_pidDriveFlags & PIDDRIVEF_PIDDRIVE_ON)
        {
            ...
            {
                m_drive->MecanumDrive_Polar(m_yPower,
                                            m_yPower,
                                            turnPower);
            }
            else if (expired ||
                     (turnOnTarget &&
                      ((m_pidDriveFlags & PIDDRIVEF_TURN_ONLY) || yOnTarget)))
            {
